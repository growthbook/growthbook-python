...
def compare(val1, val2) -> int:
    if (type(val1) is int or type(val1) is float) and not (
        type(val2) is int or type(val2) is float
    ):
        if val2 is None:
            val2 = 0
        else:
    ...
    if (type(val2) is int or type(val2) is float) and not (
        type(val1) is int or type(val1) is float
    ):
        if val1 is None:
            val1 = 0
        else:
...
def evalOperatorCondition(
    operator, attributeValue, conditionValue, savedGroups
) -> bool:
    if operator == "$eq":
        try:
        ...
        return paddedVersionString(attributeValue) == paddedVersionString(
            conditionValue
        )
    elif operator == "$vne":
        return paddedVersionString(attributeValue) != paddedVersionString(
        ...
        if type(conditionValue) is not str:
            return False
        if conditionValue not in savedGroups:
            return False
...
def _getOrigHashValue(
    eval_context: EvaluationContext | AsyncEvaluationContext,
    ...
    if actual_attr in eval_context.user.attributes:
        val = (
            ""
            if eval_context.user.attributes[actual_attr] is None
            else eval_context.user.attributes[actual_attr]
        )
    ...
    if (
        (not val or val == "")
        and fallbackAttr
        and eval_context.global_ctx.sticky_bucket_service
    ):
        if fallbackAttr in eval_context.user.attributes:
            val = (
                ""
                if eval_context.user.attributes[fallbackAttr] is None
                else eval_context.user.attributes[fallbackAttr]
            )
...
def _getHashValue(
    ...
    (attr, val) = _getOrigHashValue(
        attr=attr, fallbackAttr=fallbackAttr, eval_context=eval_context
    )
    return (attr, str(val))
...
def _isIncludedInRollout(
    ...
    (_, hash_value) = _getHashValue(
        attr=hashAttribute, fallbackAttr=fallbackAttribute, eval_context=eval_context
    )
    if hash_value == "":
        return False
...
def _isFilteredOut(
    filters: List[Filter], eval_context: EvaluationContext | AsyncEvaluationContext
) -> bool:
    for filter in filters:
        (_, hash_value) = _getHashValue(
            attr=filter.get("attribute", "id"), eval_context=eval_context
        )
        if hash_value == "":
            return False
...
def _get_sticky_bucket_experiment_key(
    experiment_key: str, bucket_version: int = 0
) -> str:
    return experiment_key + "__" + str(bucket_version)


def _get_sticky_bucket_assignments(
    evalContext: EvaluationContext | AsyncEvaluationContext,
    attr: str = None,
    fallback: str = None,
) -> Dict[str, str]:
    merged: Dict[str, str] = {}
    ...
    if key in evalContext.user.sticky_bucket_assignment_docs:
        merged = evalContext.user.sticky_bucket_assignment_docs[key].get(
            "assignments", {}
        )
    ...
    if fallback:
        ...
        if key in evalContext.user.sticky_bucket_assignment_docs:
            ...
            for k, v in (
                evalContext.user.sticky_bucket_assignment_docs[key]
                .get("assignments", {})
                .items()
            ):
                if k not in merged:
...
def _is_blocked(
    assignments: Dict[str, str], experiment_key: str, min_bucket_version: int
) -> bool:
...
def _get_sticky_bucket_variation(
    ...
    assignments = _get_sticky_bucket_assignments(
        attr=hash_attribute, fallback=fallback_attribute, evalContext=evalContext
    )
    ...
    variation = next(
        (i for i, v in enumerate(meta) if v.get("key") == variation_key), -1
    )
    if variation < 0:
        return {"variation": -1}
...
def run_experiment(
    ...
    if (
        ...
        if doc and data.get("changed", False):
            ...
            evalContext.global_ctx.sticky_bucket_service.save_assignments(doc)
...
def _generate_sticky_bucket_assignment_doc(
    ...
    existing_assignments = evalContext.user.sticky_bucket_assignment_docs.get(
        key, {}
    ).get("assignments", {})
...
def _getExperimentResult(
    ...
    (hashAttribute, hashValue) = _getOrigHashValue(
        attr=experiment.hashAttribute,
        fallbackAttr=experiment.fallbackAttribute,
        eval_context=evalContext,
    )
