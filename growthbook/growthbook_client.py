...
class EnhancedFeatureRepository(FeatureRepository, metaclass=SingletonMeta):
    ...
    async def load_features_async(
        ...
        if api_host == self._api_host and client_key == self._client_key:
            decryption_key = self._decryption_key
            ttl = self._cache_ttl
        return await super().load_features_async(api_host, client_key, decryption_key, ttl, remote_eval, payload)
        ...
        try:
            ...
            if self.options.remote_eval:
                ...
                if self.options.forced_features:
                    forced_features_for_payload = [
                        [key, value]
                        for key, value in self.options.forced_features.items()
                    ]

                payload = {
                    "attributes": self.options.global_attributes,
                    "forcedFeatures": forced_features_for_payload,
                    "forcedVariations": self.options.forced_variations,
                    "url": self.options.api_host
                }
